...
namespace Infrastructure.Data
{
    public class LibrosDbContext
    {
        ...
        public List<IM253E01Libro> List()
        {
            ...
            using (var cmd = new SqlCommand("SELECT [Id],[Autor],[Editorial],[ISBN],[Foto] FROM [IM253E00Libro]", con))
            {
                try
                {
                    con.Open();
        ...
        public IM253E01Libro Details(Guid id)
        {
            ...
            using (var con = new SqlConnection(_connectionString))
            using (var cmd = new SqlCommand("SELECT [Id],[Autor],[Editorial],[ISBN],[Foto] FROM [IM253E00Libro] WHERE [Id] = @id", con))
            {
        ...
        public void Create(IM253E01Libro libro)
        {
            ...
            using (var cmd = new SqlCommand(
                "INSERT INTO [IM253E00Libro] ([Id],[Autor],[Editorial],[ISBN],[Foto]) " +
        ...
        public void Update(IM253E01Libro libro)
        {
            using (var con = new SqlConnection(_connectionString))
            using (var cmd = new SqlCommand(
                "UPDATE [IM253E00Libro] SET [Autor] = @autor, [Editorial] = @editorial, " +
        ...
        {
            using (var con = new SqlConnection(_connectionString))
            using (var cmd = new SqlCommand("DELETE FROM [IM253E00Libro] WHERE [Id] = @id", con))
            {
                cmd.Parameters.Add("@id", SqlDbType.UniqueIdentifier).Value = id;
